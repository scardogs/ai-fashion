const DEFAULT_SCENE_WEBHOOK_URL =
  "https://n8n.srv931715.hstgr.cloud/webhook/scenetoprompts"; // https://n8n.srv931715.hstgr.cloud/webhook/scenetoprompt
...
export async function handleSceneImageSubmission(
  imageFile: File,
  ...
  try {
    ...
    console.warn(
      "Direct scene webhook POST failed, falling back to server proxy:",
      err,
    );
  }

  // Fallback: proxy through our server to avoid CORS/network issues
  const proxyRes = await fetch("/api/proxy-scene-webhook", {
    method: "POST",
    body: formData,
    signal: opts?.signal,
  });
